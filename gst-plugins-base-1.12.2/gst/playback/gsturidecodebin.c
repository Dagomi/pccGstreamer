    ...
    GstURIDecodeBin * decoder)
{
  ...
  g_print("[DAVID] -> No decoder available for type \'%s\'.", capsstr);
    ...
    if (!decoder->streams || g_hash_table_size (decoder->streams) == 0) {
      if (decoder->missing_plugin_errors) {
        ...
        GST_ELEMENT_ERROR (decoder, CORE, MISSING_PLUGIN, (NULL),
            ("no suitable plugins found DAVID:\n%s", str->str));
...
new_decoded_pad_added_cb (GstElement * element, GstPad * pad,
    GstURIDecodeBin * decoder)
{
  g_printf("[DAVID] -> new_decoded_pad_added_cb\n");
  ...
  g_printf("[DAVID] -> new decoded pad, name: <%s>, GST_PAD_NAME (pad)\n");
  GST_URI_DECODE_BIN_LOCK (decoder);
...
make_decoder (GstURIDecodeBin * decoder)
{
  ...
  g_printf("[DAVID] -> type found function\n");	
  /* re-use pending decodebin */
  if (decoder->pending_decodebins) {
	g_printf("[DAVID] -> make_decoder\n");	
	...
	g_printf("[DAVID] -> make_decoder, else\n");	

    /* now create the decoder element */
    decodebin = gst_element_factory_make ("decodebin", NULL);
	g_printf("[DAVID] -> create element decoderbin, else\n");	
    ...
    capsstr = gst_caps_to_string (decoder->caps);
  
    g_printf("[DAVID] -> CAPS \'%s\'.\n", capsstr);

    g_free (capsstr);  

    /* connect signals to proxy */
	g_printf("[DAVID] -> connect signals to proxy\n");	
...
no_typefind:
  {
	g_printf("[DAVID] -> no type find bin\n");	
